  ...
  while (Serial2.available() > 0) {
    ...
    if (bufferIndex == 0) {
      if (inData == CRSF_ADDRESS_FLIGHT_CONTROLLER) {
        ...
        uint8_t pLength = Serial2.read();
        for (uint8_t i = 0; i < pLength; i++) {
          Serial2.read();
        }
        Serial.print(F("Packet for "));
        Serial.print(inData, HEX);
        Serial.print(F(" was read. Size of packet: "));
        Serial.println(pLength);
      }
    } else if (bufferIndex == 1) {
      packetLength = Serial2.read();
      inBuffer[bufferIndex++] = packetLength;
    } else if (bufferIndex < packetLength + 1) {
      inBuffer[bufferIndex++] = Serial2.read();
    } else {
      Serial.print(F("FC packet was read. Size of packet: "));
      Serial.print(inBuffer[1]);
      Serial.print(F(" type of packet: "));
